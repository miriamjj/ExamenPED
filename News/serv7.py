...
while True:
    ...
    for s in ready_list:
        ...
        if s == socket_serv:
            ...
            N = s.recv(1024)
            noticias = modelo.devuelve_noticias(int(N.decode('utf8')))
            while True:
                for linea in noticias:
                    if not linea:
                        socket_cli.send(mensaje.encode('utf8'))
                        print('Enviados articulo de ', N.decode('utf8'),f'dias al cliente {dir_cli[0]}:{dir_cli[1]}' )
                        break
                socket_cli.send(mensaje.encode('utf8'))
            ...
            while True:
                ...
                if not data: # Sale del bucle si ha llegado a un end of file (EOF)
                    break
                articulo += data.decode('utf8').strip() + "\n"

            modelo.publica_noticia(datetime.date.now(), titulo.decode('utf8'), nick.decode('utf8'), articulo.decode('utf8'))
            #usuario = clientes[s]
            noticia = modelo.distribuye_noticia()
